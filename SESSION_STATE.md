...
- **Combined VANC source cleanup — in review.** Continue human review of `packet-smpte-291-vanc.c` after consolidation. Current cleanup includes consistent declaration grouping, major section headers, registration-array formatting, named DID/SDID constants, protocol-qualified internal symbols, and removal of trivial helpers that duplicate Wireshark APIs.
- **ST 12-2 short payload handling — resolved in `review3`.** Short payloads now create the ST 12-2 protocol item, emit malformed expert information, and return instead of silently returning 0.
- **ST 2016-3 fixed-length handling — resolved in `review3`.** Any payload length other than 8 bytes now emits the existing malformed expert indication and returns immediately; redundant later `len < 1/4/8` checks were removed.
