...
- Merged release-4.4 MR !19576 independently demonstrates the same rule for reassembly. QUIC STREAM reassembly was changed from packet addresses/ports to a protocol-semantic key containing the QUIC connection number, stream ID, direction, and reassembly ID. The rationale explicitly notes endpoint migration and the possibility that encapsulated protocol dissection can change `packet_info` addresses/ports within a datagram. Reassembly identity therefore must remain stable even when transport presentation state changes.
