...
- Treat static-analysis/warning cleanup as a semantic code change that needs regression validation, not as mechanically safe churn. Merged MR !25547 repaired a severe IEEE 802.15.4 CCM stack-buffer underflow introduced when a previous cppcheck cleanup changed pointer syntax *and* accidentally changed array indices/variables at the same time. The repair deliberately preserved the analyzer-motivated style change while restoring the original indexing semantics, then validated OSCORE decryption, IEEE 802.15.4 and ZigBee captures, a warning-free build, and cppcheck. When satisfying a warning, minimize simultaneous semantic edits and retest representative callers/edge cases of the touched code.
