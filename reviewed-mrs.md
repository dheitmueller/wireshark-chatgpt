...
| !25776 | Scanned | Stratoshark Find-in-Packet behavior synchronized with Wireshark: selected-frame state is propagated to SearchFrame and cancel/mode behavior is aligned. Merged; no substantive human review. |
...
| !25778 | Deep | K12 wiretap buffer-overrun fix. Separates file-offset advancement from bytes actually written to the record buffer by replacing a raw pointer/length pair with `Buffer` and using `ws_buffer_length()`. Durable lesson: source bytes consumed and destination bytes produced must not share one length semantic when framing bytes are skipped. Added to `dissector-conventions.md`. |
| !25779 | Discussion-focused (closed/superseded) | First PortableApps plugin-preservation submission accidentally carried an already-merged unrelated commit. Author closed it specifically to resubmit from a clean current-master branch; !25781 is the later merged clean submission. Corroborates clean topic-branch/scope discipline rather than serving as an implementation exemplar. |
